...
class aws:
    def dns(accounts, iamrole):
        ...
        with tqdm(total=total_accounts, desc="Processing accounts", unit="account") as account_pbar:
            for account_idx, aws_account in enumerate(accounts, 1):
                account_pbar.set_postfix_str(f"Account {account_idx}/{total_accounts}: {aws_account}")
                ...
                if len(str(aws_account)) == 12:
                    ...
                    # Count total zones first
                    zones_list = []
                    for page in paginator_zones.paginate():
                        zones_list.extend(page.get("HostedZones", []))
                    
                    # Use the paginator to iterate through all pages of hosted zones
                    with tqdm(total=len(zones_list), desc=f"  Zones in {aws_account}", unit="zone", leave=False) as zone_pbar:
                        for zone in zones_list:
                            zone_name = zone.get("Name", "unknown")
                            zone_pbar.set_postfix_str(f"Processing {zone_name}")
                    ...
                    print(f"⚠️  Invalid AWS Account number: {aws_account} (must be 12 digits)")
                    account_pbar.update(1)
        ...
        with tqdm(total=total_accounts, desc="Processing accounts", unit="account") as account_pbar:
            for account_idx, aws_account in enumerate(accounts, 1):
                ...
                if len(str(aws_account)) == 12:
                    ...
                    with tqdm(total=total_regions, desc=f"  Regions in {aws_account}", unit="region", leave=False) as region_pbar:
                        for region_idx, r in enumerate(regions, 1):
                            region_pbar.set_postfix_str(f"Region {region_idx}/{total_regions}: {r}")
                            try:
                                client = boto3.client(
                                    "ec2",
                                    aws_access_key_id=credentials["AccessKeyId"],
                                ...
                                region_pbar.set_postfix_str(f"❌ Error in {r}: {str(e)[:50]}")
                                region_pbar.update(1)
                                continue
                    ...
                    try:
                        ...
                        print(f"⚠️  Could not collect CloudFront/ACM data from account {aws_account} in us-east-1: {e}")
                        pass
