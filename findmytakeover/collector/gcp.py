...
class gcp:
    def dns(projects, path):
        ...
        with tqdm(
            total=total_projects, desc="Processing GCP projects", unit="project"
        ) as pbar:
            for proj_idx, proj in enumerate(projects, 1):
                pbar.set_postfix_str(f"Project {proj_idx}/{total_projects}: {proj}")
                ...
                for managed_zone in managed_zones:
                    ...
                    for resource_record_set in resource_record_sets:
                        if (
                            resource_record_set.record_type == "A"
                            or resource_record_set.record_type == "AAAA"
                        ):
                            for ip_address in resource_record_set.rrdatas:
                                dnsdata.append(
                                    [proj, resource_record_set.name[:-1], ip_address]
                                )
    ...
    def infra(projects, path):
        ...
        with tqdm(
            ...
            for proj_idx, proj in enumerate(projects, 1):
                ...
                compute_client = compute_v1.ForwardingRulesClient(
                ...
                URL = function_client.list_functions(
                ...
                for i in URL.functions:
                    infradata.append(
                        [
                            proj,
                            "cloudfunction",
                            str(i.service_config.uri).replace("https://", ""),
                        ]
                    )
                        ...
                        for instance in response.instances:
                            for network in instance.network_interfaces:
                                for ip in network.access_configs:
                                    infradata.append(
                                        [proj, "virtualmachine", ip.nat_i_p]
                                    )
